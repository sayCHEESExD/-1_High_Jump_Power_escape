...
export class WinFlight {
  ...
  constructor(parent: HTMLElement) {
    ...
    this.root.className = 'hj-flight';
    parent.appendChild(this.root);

    for (let i = 0; i < COUNT; i += 1) {
      const node = document.createElement('img');
      node.className = 'hj-flight__cup';
  ...
  play(fromX: number, fromY: number): void {
    const target = document.querySelector('.hj-wins__icon');
    if (!target) return;
    ...
    for (let i = 0; i < COUNT; i += 1) {
      ...
      node.style.setProperty('--hj-fx', `${startX}px`);
      node.style.setProperty('--hj-fy', `${startY}px`);
      node.style.setProperty('--hj-tx', `${toX}px`);
      node.style.setProperty('--hj-ty', `${toY}px`);
      ...
      node.style.setProperty('--hj-mx', `${(startX + toX) / 2}px`);
      node.style.setProperty('--hj-my', `${Math.min(startY, toY) - 70 - Math.random() * 50}px`);
      node.style.animationDelay = `${i * STAGGER}s`;

      node.hidden = false;
      node.classList.remove('hj-flight__cup--run');
      ...
      node.classList.add('hj-flight__cup--run');
